...
public:
    ...
    int load_net(const string &path);
    ...
    void mutate() {
        double l = Random::random_value(0, layers.size() - 1);
        double n = Random::random_value(0, layers[l].neurons.size() - 1);
        double w = Random::random_value(0, layers[l].neurons[n].weights.size() - 1);
    ...
    for (int l = 0; l < model.size(); l++) {
        ...
        for (int n = 0; n < model[l]; n++) {
            ...
            for (int w = 0; l != model.size() - 1 && w < model[l + 1]; w++) {
                Weight weight;
                weight.value = Random::random_value(static_cast<int>(min), static_cast<int>(max));
...
int NeuralNet::load_net(const string &path) {
    ifstream in(path);
...
int NeuralNet::save_net(const string &path) {
    ofstream out(path);
    for (auto &layer : layers) {
        for (auto &neuron : layer.neurons) {
            for (auto &weight : neuron.weights) {
        ...
        for (int neuron = 0; neuron < layers[layer].neurons.size(); neuron++) {
            if (layer < layers.size() - 1) {//esclude il bias
                if (neuron != layers[layer].neurons.size() - 1) {
                    double sum = 0;
                    for (auto &previus_neuron : layers[layer - 1].neurons) {
                        sum += previus_neuron.value * previus_neuron.weights[neuron].value;
                    }
