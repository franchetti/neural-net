...
void writeTxt(string text, int x, int y, int width, int height, double angle, SDL_Renderer* renderer) {
	...
	SDL_RenderCopyEx(renderer, Message, NULL, &Message_rect, angle, NULL, SDL_FLIP_NONE); //you put the renderer's name first, the Message, the crop size(you can ignore this if you don't want to dabble with cropping), and the rect which is the size and coordinate of your texture
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				Sans = TTF_OpenFont("C:/Sans.ttf", 24); //this opens a font style and sets a size
...
public:
	...
	int button() {
		...
		if (point_inside_rect(SDL_Point{ (float)mx,(float)my }, button)) {
			return 10;
		}
		else if (point_inside_rect(SDL_Point{ (float)mx,(float)my }, button1)) {
			return -10;
		}
...
void SDLnet::draw_graph(vector<double> points, SDL_Rect area, SDL_Renderer* renderer, double zoom) {
	if (points.size() > 0) {
		...
		rect.w = area.w - 1;
		SDL_RenderDrawRect(renderer, &rect);
		int y1 = area.h - (points[0] * scale);
		for (int i = 1; i < points.size(); i++) {
			int y2 = area.h - (points[i] * scale);
...
void SDLnet::draw(NeuralNet* net, Simulation* simulation, vector<double> points, SDL_Renderer* renderer) {
	...
	if (point_inside_rect(SDL_Point{ (float)mx,(float)my }, net_area)) {
	...
	if (point_inside_rect(SDL_Point{ (float)mx,(float)my }, simulation_area)) {
		SDL_SetRenderDrawColor(renderer, 255, 0, 0, 125);
		SDL_RenderFillRect(renderer, NULL);
		SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
		...
		if (wheel != 0) {
			...
			else { simulation_zoom-= .1; if (simulation_zoom <= 0)simulation_zoom = 1;}
			wheel=0;
		}
	}

	draw_graph(points, graph_area, renderer, graph_zoom);
	if (point_inside_rect(SDL_Point{ (float)mx,(float)my }, graph_area)) {
		SDL_SetRenderDrawColor(renderer, 255, 0, 0, 125);
		SDL_RenderFillRect(renderer, NULL);
		SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
		if (wheel != 0) {
	...
	if (point_inside_rect(SDL_Point{ (float)mx,(float)my }, log_area)) {
		SDL_SetRenderDrawColor(renderer, 255, 0, 0, 125);
		SDL_RenderFillRect(renderer, NULL);
		SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
	...
	if (point_inside_rect(SDL_Point{ (float)mx,(float)my }, logo_area)) {
		SDL_SetRenderDrawColor(renderer, 255, 0, 0, 125);
		SDL_RenderFillRect(renderer, NULL);
		SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
	}
	...
	if (point_inside_rect(SDL_Point{ (float)mx,(float)my }, option_area)) {
		SDL_SetRenderDrawColor(renderer, 255, 0, 0, 125);
		SDL_RenderFillRect(renderer, NULL);
		SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
	}
